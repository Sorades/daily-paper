...
impl FileStateStore {
    ...
    pub fn ensure_dirs(&self) -> Result<()> {
        for subdir in &["state/runs", "state/zotero/snapshots", "cache/embeddings", "cache/rerank", "cache/papers", "reports", "deliveries/history"] {
...
mod tests {
    ...
    fn ensure_dirs_creates_structure() {
        ...
        assert!(store.root().join("state/runs").is_dir());
